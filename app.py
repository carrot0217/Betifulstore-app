...
def user_request_ajax():
    ...
    """, (user_id, item_id, quantity, "대기 중", session.get('store')))
...
def login():
    if request.method == 'POST':
        ...
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT password, is_admin, store FROM users WHERE username = %s", (user_id,))
        result = cur.fetchone()
        cur.close()
        conn.close()

        if result:
            db_password, is_admin, store = result
            print("회원 로그인 - 매장명:", store)  # 확인위한 debug print
            if password == db_password:
                session['user_id'] = user_id
                session['is_admin'] = is_admin
                session['store_name'] = store  # 대입 값 확인
...
def user_orders():
    ...
    return render_template('user_orders.html', orders=orders, today=today, item_list=item_list, request=request)
...
def user_home():
    ...
    print("대입된 store_name:", store_name)  # debug print

    conn = get_connection()
    cur = conn.cursor()

    # 🔹 쪽지 수신자 리스트 (username 기준)
    cur.execute("""
        SELECT username, store_name
        FROM users
        WHERE is_admin = false AND store_name IS NOT NULL AND username != %s
        ORDER BY store_name
    """, (username,))
    recipients = cur.fetchall()  # [(username, store_name), ...]
    ...
    return render_template(
        ...
        recipients=recipients,   # ✅ 쪽지 수신자 전달
        messages=messages        # ✅ 받은 쪽지 목록 전달
    )
# ----------------------- 관리자페이지 매장명 수정 -----------------------
...
def update_store_name():
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("UPDATE users SET store_name = %s WHERE username = %s", (store_name, username))
    conn.commit()
    cur.close()
    conn.close()
...
def message_detail(msg_id):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT sender_id, subject, content, sent_at FROM messages WHERE id = %s AND receiver_id = %s", (msg_id, user_id))
    msg = cur.fetchone()
    # 읽음 처리
    ...
    if request.method == 'POST':
        ...
        if not username or not password or not store:
            ...
            if cur.fetchone():
                ...
                cur.execute("""
                    INSERT INTO users (username, password, store, is_admin)
                    VALUES (%s, %s, %s, %s)
                """, (username, password, store, is_admin))
                conn.commit()
                flash("✅ 사용자 등록 완료")
...
def edit_user_store(user_id):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("UPDATE users SET store = %s WHERE id = %s", (store, user_id))
    conn.commit()
    cur.close()
    conn.close()
    flash("🏪 매장 정보가 수정되었습니다.")
    return redirect(url_for('manage_users'))
